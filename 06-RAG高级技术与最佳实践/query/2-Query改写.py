...
class QueryRewriter:
    def rewrite_context_dependent_query(self, current_query, conversation_history):
        ...
        instruction = """
你是一个智能的查询优化助手。
请分析用户的当前问题以及前序对话历史，判断当前问题是否依赖上下文。
...
"""
        ...
        response = get_completion(instruction, user_message)
        try:
...
"""
    ...
    """
        ...
        if result and isinstance(result, dict):
            # 确保包含必要的键
            if "query_type" in result and "rewritten_query" in result:
                return result
    ...
    def auto_rewrite_and_execute(self, query, conversation_history="", context_info=""):
        """ 自动识别 Query 类型并进行改写，然后根据类型调用相应的改写方法 """
        # 进行自动识别
